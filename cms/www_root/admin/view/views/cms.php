<?php
	...
	class Cms extends Visual {
		...
		public function __construct($module, $website_title) {
		...
		public function render() {
			...
			if (!is_null($this->_module)) {
				$actions_menu = new ActionsMenu($this->_module->getActionButtons());
				$template_engine->assign("actions_menu", $actions_menu->render());
				$template_engine->assign("page_title", $this->_module->getTitle());
				$template_engine->assign("module_head_includes", $this->_module->getHeadIncludes());
			}
			...
			$template_engine->assign("content_pane", $this->renderContentPane($this->_module));
		...
		private function renderContentPane() {
			$template_engine = TemplateEngine::getInstance();
			if (!is_null($this->_module)) {
				return $this->_module->render($this);
			} else {
