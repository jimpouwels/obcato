                        ...
                        {else}
                            {foreach from=$group.modules item=module}
                                <li{if $module.last} class="last"{/if}>
                                    ...
                                    <a {if {$module.popup}}onclick="window.open('/admin/popup_entity.php?module_id={$module.id}','{$module.title}','width=640,height=480'); return false;"{/if} href="{$backend_base_url_raw}?module_id={$module.id}">{$module.title}</a>
                                </li>
                            {/foreach}
                        {/if}
					</ul>
