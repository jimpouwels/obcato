<div class="link-lookup" id="link-lookup-{$field_name}">
    ...
    <div id="link-lookup-empty-{$field_name}" {if $selected_link_id}style="display:none;"{/if}>
        <button type="button" class="link-lookup-trigger-btn"
                onclick="openLinkPickerModal(function(link) { selectLinkLookup('{$field_name}', link.id, link.title, link.url); });">
            &#128279; Herbruikbare link selecteren&hellip;
        </button>
    </div>
    ...
    <div class="link-lookup-preview" id="link-lookup-preview-{$field_name}" {if !$selected_link_id}style="display:none;"{/if}>
        <div class="link-lookup-preview-info">
            <div class="link-lookup-preview-title" id="link-lookup-title-{$field_name}">{$selected_link_title|escape}</div>
        ...
        <div class="link-lookup-preview-actions">
            <button type="button" class="link-lookup-preview-change"
                    onclick="openLinkPickerModal(function(link) { selectLinkLookup('{$field_name}', link.id, link.title, link.url); });">Wijzig</button>
            <button type="button" class="link-lookup-preview-clear"
