...
class PageDaoMysql implements PageDao {
    ...
    private static string $myAllColumns = "e.id, e.template_id, e.last_modified, e.name, e.title, e.published, e.scope_id,
                      e.created_at, e.created_by, e.type, e.version, p.navigation_title, p.url_title, p.description, p.seo_title, p.parent_id, p.show_in_navigation,
    ...
    public function updatePage(Page $page): void {
        $query = "UPDATE pages SET navigation_title = ?, show_in_navigation = ?, include_parent_in_url = ?, include_in_searchindex = ?, follow_up = ?, `description` = ?, url_title = ?, seo_title = ? WHERE element_holder_id = ?";
        ...
        $statement->bind_param('siiiisssi', $navigationTitle, $showInNavigation, $includeParentInUrl, $includeInSearchEngine, $followUp, $description, $urlTitle, $seoTitle, $elementHolderId);
        $this->mysqlConnector->executeStatement($statement);
